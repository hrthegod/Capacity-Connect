...
const MySkills = () => {
  ...
  useEffect(() => {
    apiFetch("/competencies/profile", {}, "LEARNER")
      .then((res) => {
      ...
      .catch((err) => {
        console.error("Failed to fetch learner competencies:", err);
        setCompetencies([]);
      })
      .finally(() => {
        setLoading(false);
  ...
  const stats = {
    ...
    advancedSkills: competencies.filter((c) => c.current_level === "ADVANCED").length,
    intermediateSkills: competencies.filter((c) => c.current_level === "INTERMEDIATE").length,
    beginnerSkills: competencies.filter((c) => c.current_level === "BEGINNER" || !c.current_level).length,
    averageProficiency: competencies.length > 0 ? 80 : 0,
    skills: competencies,
  };
