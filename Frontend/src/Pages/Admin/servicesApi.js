...
const DEFAULT_CONFIG = {
  ...
  dbDialect:
    localStorage.getItem(STORAGE_KEYS.DB_DIALECT) || "PostgreSQL / Custom REST",
  mode: localStorage.getItem(STORAGE_KEYS.MODE) || "auto",
};

export const getBackendConfig = () => {
  return {
    baseUrl:
      localStorage.getItem(STORAGE_KEYS.BASE_URL) || DEFAULT_CONFIG.baseUrl,
    token: localStorage.getItem(STORAGE_KEYS.TOKEN) || DEFAULT_CONFIG.token,
    dbDialect:
      localStorage.getItem(STORAGE_KEYS.DB_DIALECT) || DEFAULT_CONFIG.dbDialect,
...
export const setBackendConfig = (config) => {
  ...
  if (config.dbDialect !== undefined)
    localStorage.setItem(STORAGE_KEYS.DB_DIALECT, config.dbDialect);
...
export async function apiRequest(endpoint, options = {}) {
  ...
  // Get the actual JWT created during login
  ...
  try {
    ...
    console.warn(
      `[API] Request to ${fullUrl} could not complete:`,
      err.message,
    );
...
export async function testBackendConnection(customUrl, customToken) {
  ...
  const tokenToUse =
    customToken !== undefined ? customToken : getBackendConfig().token;
  ...
  return {
    ...
    message:
      "Could not connect to backend server. Verify the server is running and CORS is enabled.",
  };
}
    ...
    if (config.mode !== "fallback") {
      try {
        ...
        if (res.data && Array.isArray(res.data.data)) {
          setLocalData(STORAGE_KEYS.USERS, res.data.data);
...
export const trainingService = {
  ...
  async delete(id) {
    ...
    if (config.mode !== "fallback") {
      try {
        await apiRequest(`/training-programs/${id}`, {
          method: "DELETE",
        });
      } catch {}
    }
...
export const competencyService = {
  ...
  async delete(id) {
    ...
    if (config.mode !== "fallback") {
      try {
        await apiRequest(`/competencies/${id}`, {
          method: "DELETE",
        });
      } catch {}
    }
...
export const knowledgeService = {
  ...
  async delete(id) {
    ...
    if (config.mode !== "fallback") {
      try {
        await apiRequest(`/knowledge-hub/${id}`, {
          method: "DELETE",
        });
      } catch {}
    }
    ...
    if (config.mode !== "fallback") {
      try {
        const res = await apiRequest(
          `/analytics/organization?timeframe=${timeframe}`,
        );
